...
def dsgroupview(request, groupID, loanID, taskId, processInstanceId):
    ...
    print "grp:"
    print groups[0]
    return render_to_response('ds_groupview.html',
                              {"groupId": groupID, "loanId": loanID, "processInstanceId": processInstanceId,
                               "taskId": taskId, "group": groups[0], "user": username})
...
def groupViewQuery2(request, groupID, loanID, taskId, processInstanceId):
    ...
    print "grp:"
    print groups[0]
    return render_to_response('queryResponseDS.html',
                              {"groupId": groupID, "loanId": loanID, "processInstanceId": processInstanceId,
                               "taskId": taskId, "group": groups[0], "user": username})
...
def DocumentView(request, groupId):
    ...
    try:
        bodyData = {"groupId": str(groupId)}
        serialized_data = sscoreClient._urllib2_request('workflowDetailView/GroupDocument', bodyData,
                                                        requestType='POST')
        return HttpResponse(json.dumps(serialized_data), content_type="application/json")
    except ShgInvalidRequest, e:
        ...
        if request.method == "POST":
            ...
            if memberValResponse["data"]["status"] != "fail":
                if formData.has_key("message"):
                    ...
                    print "commentUpdate"
                    print commentUpdate
                dataUpdateResponse = sscoreClient._urllib2_request('workflowEdit/updateMemberGroupLoan',
                                                                   bodyDataUpdation, requestType='POST')
