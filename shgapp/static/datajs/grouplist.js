...
$(function() {
    $("a[href=#groupDetail]").click(function() {
        loadGroupRoles(groupId, loanId, taskName);
    });
     $("a[href=#memberConfHistory]").click(function() {
        var memberId = document.getElementById("memberId").innerHTML;
        conflictList(memberId);
    });
    $("a[href=#fsr]").click(function() {
        var memberId = document.getElementById("memberId").innerHTML;
    ...
    $("a[href=#viewDocuments]").click(function() {
        documentView(loanId);
    });
    $("a[href=#SHGCreditHistory]").click(function() {
...
function highMarksList(memberId,loanId,memberDocData) {
     $.ajax({
...
function conflictList(memberId) {
     var conflictListArr = [];
    $.ajax({
        url: '/getmemberConflictHist/' + memberId,
        dataType: 'json',
        type: "POST",
        beforeSend: function() {
            $("#loading").show();
...
function getLoanDetailsDBNonEditable(groupId, loanId) {
    $.ajax({
    ...
    if(disbDocData && disbDocData[0]){
        for(var key in disbDocData){
            ...
            if (disbDocData[key]["modeOfPayment"] == "null"){
                disbDocData[key]["modeOfPayment"] = 'No Data';
            }
            obj[disbDocData[key]["appMemberId"]+"_modeOfPayment"] = disbDocData[key]["modeOfPayment"];
            if (disbDocData[key]["bank"] == "null"){
                disbDocData[key]["bank"] = 'No Data';
            }
            obj[disbDocData[key]["appMemberId"]+"_bank"] = disbDocData[key]["bank"];
            obj[disbDocData[key]["appMemberId"]+"_memberAvailedLoan"] = disbDocData[key]["memberAvailedLoan"];
            selectOptValArray.push(obj);
            if(disbDocData[key]["chequeNo"] == null){
                disbDocData[key]["chequeNo"] = 'No Data'
            }
            if(disbDocData[key]["modeOfPayment"] == null){
                disbDocData[key]["modeOfPayment"] = 'No Data'
            }
            ...
            if(disbDocData[key]["memberAvailedLoan"] == null){
                memberAvailedLoan = 'No Data';
            }
...
function getGroupLevelInfo(groupId,loanId,taskName){
    var memId;
    var totalCount = 0;
    var rejCount = 0;
    $("#groupMembersDropDown").empty();
    $.ajax({
        url: '/getGroupLevelInfo/' + groupId + '/' + loanId + '/'+taskName,
        dataType: 'json',
        beforeSend: function() {
            $("#loading").show();
        },
